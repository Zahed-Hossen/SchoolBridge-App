...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const userRole =
        signupType === 'visitor' ? ROLES.VISITOR : role || ROLES.PLATFORM_USER;
      ...
      res.status(HTTP_STATUS.CREATED).json(
        createResponse(true, MESSAGES.AUTH.SIGNUP_SUCCESS, {
          ...
          // (Removed duplicate/erroneous code block after signup route)
          ...tokens,
        }),
      );
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      if (user) {
        // Existing user
        if (user.role !== role) {
          console.log('❌ Google user role mismatch:', {
            expected: role,
            actual: user.role,
          });
          return res
